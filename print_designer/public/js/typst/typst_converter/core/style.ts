...
export function collectBoxSpacing(style: any, prefix: string = "margin"): string | null {
  const dirs = ["Top", "Right", "Bottom", "Left"];
  const out: Record<string, string> = {};
  if (style[prefix]) {
    const v = parseCssSize(style[prefix]);
    dirs.forEach(dir => out[dir.toLowerCase()] = v);
  }
  dirs.forEach(dir => {
...
export function rectangleStyleToTypst(styleObj: StyleObject): string {
  ...
  for (const [key, value] of Object.entries(styleObj)) {
    if (value === "" || value === "none" || value === undefined || value === null) continue;
...
export function dynamicLabelStyleToTypst(labelStyleObj: StyleObject): string {
  ...
  for (const [key, value] of Object.entries(labelStyleObj)) {
    ...
    switch (key) {
      case "color":
        if (typeof value === "string") {
          lines.push(`fill: rgb("${cssColorWithOpacityToTypstHex(value, labelStyleObj.opacity)}")`);
        }
        break;
      case "fontFamily":
        if (typeof value === "string") {
      ...
      case "fontSize":
        ...
        lines.push(`size: ${toPt(typeof value === "number" ? value : String(value))}`);
        break;
      case "fontWeight":
        // Accept both string and number
        lines.push(`weight: ${typeof value === "number" ? value : `"${value}"`}`);
        break;
      case "letterSpacing":
        lines.push(`tracking: ${toPt(typeof value === "number" ? value : String(value))}`);
        break;
    }
  }
...
export function textOrTableStyleToTypst(styleObj: StyleObject): string {
  ...
  for (const [key, value] of Object.entries(styleObj)) {
    ...
    switch (key) {
      case "color":
        if (typeof value === "string") {
          lines.push(`fill: rgb("${cssColorWithOpacityToTypstHex(value, styleObj.opacity)}")`);
        }
        break;
      case "backgroundColor":
        if (typeof value === "string") {
          lines.push(`highlight: rgb("${cssColorWithOpacityToTypstHex(value, styleObj.opacity)}")`);
        }
        break;
      case "fontFamily":
        if (typeof value === "string") {
  ...
  for (const [key, value] of Object.entries(styleObj)) {
    ...
    switch (key) {
      case "color":
        ...
        lines.push(`fill: rgb("${cssColorWithOpacityToTypstHex(value as string, styleObj.opacity)}")`);
        break;
      case "fontFamily":
        lines.push(`font: "${value}"`);
        break;
