...
export const getGridMinimumRowHeight = (grid = {}) => {
	return GRID_MIN_ROW_HEIGHT;
};
...
export const getGridHeight = (grid = {}) =>
	(Array.isArray(grid.rowHeights) ? grid.rowHeights : []).reduce(
		(total, rowHeight) => total + Math.max(GRID_MIN_ROW_HEIGHT, numericCssValue(rowHeight)),
		0
	);
...
export const setGridColumnWidth = (grid, columnIndex, width) => {
	normalizeGridStructure(grid);
	if (columnIndex < 0 || columnIndex >= grid.columns) return;
	const columnWidth = numericCssValue(width);
	if (!Number.isFinite(columnWidth) || columnWidth <= 0) return false;
	grid.columnWidths[columnIndex] = Math.max(GRID_MIN_COLUMN_WIDTH, columnWidth);
...
export const mergeGridCellRight = (grid, cell) => {
	...
	if (!targetCell || targetCell.mergedInto || targetCell.rowSpan != sourceCell.rowSpan)
		return false;
...
export const mergeGridCellDown = (grid, cell) => {
	...
	if (!targetCell || targetCell.mergedInto || targetCell.colSpan != sourceCell.colSpan)
		return false;
