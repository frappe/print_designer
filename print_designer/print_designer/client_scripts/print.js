frappe.pages["print"].on_page_load = function (wrapper) {
    if (
        !window.localStorage["desk_assets:assets/print_designer/js/pdf.min.js"]
    ) {
...
frappe.ui.form.PrintView = class PrintView extends frappe.ui.form.PrintView {
    ...
    make() {
		...
		this.header_prepend_container = $(`<div class="print_selectors flex col align-items-center"></div>`).prependTo(
		...
		this.toolbar_print_format_selector = frappe.ui.form.make_control({
			...
			change: () => {
				if (this.toolbar_print_format_selector.value == this.toolbar_print_format_selector.last_value) return;
		...
		this.toolbar_language_selector = frappe.ui.form.make_control({
			df: {
				...
				options: "Language",
				change: () => {
					if (this.toolbar_language_selector.value == this.toolbar_language_selector.last_value) return;
    ...
    async designer_pdf(print_format) {
        ...
        canvasContainer.innerHTML = `${frappe.render_template(
            "print_skeleton_loading"
        )}`;
    ...
    printit() {
        ...
        if (this.get_print_format().print_designer) {
            ...
            this.pdfDoc.getData().then((arrBuff) => {
                ...
                let iframe;
                let iframeAvailable =
                    document.getElementById("blob-print-iframe");
                if (!iframeAvailable) {
    ...
    show(frm) {
        ...
        this.inner_msg = this.page.add_inner_message(`
				<a style="line-height: 2.4" href="/app/print-designer?doctype=${
                    this.frm.doctype
                }">
    ...
    setup_toolbar() {
        ...
        this.pdf_btn = this.page.add_button(
            __("PDF"),
            () => this.render_pdf(),
            { icon: "small-file" }
        );
